...
bool CanBeAdjacent(int mask, const std::vector<int>& v, int n) {
  for (int i = 0; i < n; ++i) {
...
int CountDistributions(int n, int m, const std::vector<std::vector<int>>& v, const int kMod) {
  std::vector<std::vector<int>> dp(m, std::vector<int>(1 << n, 0));
  for (int mask = 0; mask < (1 << n); ++mask) {
    if (CanBeAdjacent(mask, v[0], n)) {
      dp[0][mask] = 1;
    }
  }
  for (int i = 1; i < m; ++i) {
    for (int mask = 0; mask < (1 << n); ++mask) {
      if (!CanBeAdjacent(mask, v[i], n)) {
        continue;
      }
      std::vector<int> prev_masks = PrevMasks(mask, n);
      for (int prev_mask : prev_masks) {
        if (CanBeAdjacent(prev_mask, v[i - 1], n)) {
...
int main() {
  ...
  std::cout << CountDistributions(n, m, v, kMod) << "\n";
}
